...
```{r}
tmp <- table(lycee$MEFSTAT11)
tmp[tmp>1]

tmp <- table(lycee$Diplôme)
tmp[tmp>1]

```

On a beaucoup de valeurs pour le **diplôme**, la **Spécialité libellé** etc. 
De même pour les colonnes **MEFSTAT** et **Code Diplôme**.

La variable __première année__ n'est pas très utile non plus car elle indique 
uniquement 2019.
...
```{r}
lycee %>% select(-`Année diplôme`,-`Spécialité libellé`,
                 -starts_with("MEFSTAT"),
                 -starts_with("Code Diplôme"),
...
libellé de diplôme, MEFTSTAT, ... Ce qui est curieux

```{r}
a=table(lycee$`Spécialité libellé`)
a[a>1]
```
